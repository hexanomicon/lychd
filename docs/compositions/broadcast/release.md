...
Release is the deliberate boundary between a reviewable local candidate and an external effect.
An accepted package can remain local forever without being incomplete.

## Review before effect

Source support, claim approval, voice and likeness authority, privacy and Portal use,
accessibility, asset admission, and render validation each have an exact gate. Publication
additionally pins the candidate digest, destination, audience, visibility, schedule, disclosures,
cost, and approving Principal.
...
revisions. Source snapshots, rejected renders, candidates, destination receipts, and analytics keep
separate retention. Deletion inventories derivatives and requests authorized remote removal while
preserving a content-free receipt; it cannot promise that caches or feeds forgot a published copy.
