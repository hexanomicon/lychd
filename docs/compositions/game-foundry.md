...
| **Application begins with** | an admitted design contract, pinned source revision, engine profile, tests, and optional `VisualAssetBundle@1` or `SonicAssetBundle@1` |
...
Voidlight's visual lineage, Riffmaw's sonic lineage, a game engine, a store account, or another
...
2. **Admit assets.** Validate exact `VisualAssetBundle@1` and `SonicAssetBundle@1` inputs against
   the project target, rights, semantic roles, and spatial or temporal limits; return findings
   instead of editing either bundle.
...
`game.import_asset_bundle@1`, `game.build_candidate@1`,
...
| Source assets | Voidlight and Riffmaw bundle manifests, admitted by exact digest |
...
small visual bundle and one small sonic bundle, import them through a test adapter, build one
playable scene, run one declared controller scenario, and emit exactly one
`PlayableBuildBundle@1` with source, build, test, and playtest receipts. Signing, upload, store
accounts, telemetry export, public players, and release remain outside the proof.

Related: [Voidlight](voidlight-studio.md) · [Riffmaw](riffmaw.md) · [Workflow](../adr/28-workflow.md) ·
