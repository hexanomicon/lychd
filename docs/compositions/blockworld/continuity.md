...
A cold backup follows an orderly save and stop. The Magus alone restores a named snapshot, and
restoration increments the world epoch so old leases and assumptions cannot pass as current.
Server, protocol, Sentinel, bridge, controller, Pattern, and application schemas version
...
placement from the Sentinel sequence and final plot diff, then interrupt the bridge after one
