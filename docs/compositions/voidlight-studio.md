...
material into a visual package whose images, models, textures, and motion can be inspected,
revised, and handed to another craft without losing their origin. The Magus remains the visual
director throughout.
...
| --- | --- |
| **Identity** | `voidlight.studio` revision `3` |
| **Principal Pattern** | `voidlight.build_visual_package@1` |
| **Application begins with** | an admitted commission, frozen reference dossier, rights posture, and target profile |
| **Application can return** | one approved, immutable, rights-aware `VisualAssetBundle@1`, or an exact non-completion |
| **Application stops before** | sound production, engine-native import, audiovisual or editorial assembly, public release, or rights certification |
...
3. **Specify the work.** Give every requested image, model, texture, animation, or visual sequence
   a semantic role and concrete spatial, temporal, and format constraints.
...
6. **Accept and hand off.** Seal the manifest, approvals, lineage, validators, and checksums in a
   `VisualAssetBundle@1`; the consumer performs its own admission against the exact digest.
...
`voidlight.forge_model_asset@1`, `voidlight.forge_animation_set@1`, and
`voidlight.forge_visual_sequence@1`.
`voidlight.export_visual_package@1` assembles the handoff;
`voidlight.review_visual_package@1`, `voidlight.revise_from_correction@1`, and
...
| `VisualAssetBundle@1` | bundle revision and digest, target profile, asset manifest, constraints, provenance, validators, findings, and approval |
...
Reference rights are checked before use. Likeness consent is scoped to identity, purpose,
...
accepted `VisualAssetBundle@1`. The proof uses no sound production, engine import, channel render,
Portal call, paid generation, or public release.
