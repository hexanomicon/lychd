...
!!! abstract "Context and Problem Statement"
    ...
    Like the legions of antiquity, this is not a democracy of equals. It is a hierarchy of purpose — many bodies, one Magus-signal, one Lich-pattern bound by a single Phylactery.

    If the **[Prophecy](../index.md)** is forty-two keys across the Infinite Naught, this covenant asks the obvious next question: how does one sovereign life persist across that naught without forking its soul?
...
- **Centralized State:** One Phylactery per Magus. Thralls connect to the Master's database over the network. No replication, no sharding, no consensus.
...
!!! failure "Option 1: Centralized GPU Clustering (Kubernetes / K3s / Ray / Slurm)"
    ...
    K3s is lighter but still carries the philosophical disease: it assumes a *cluster* that a *scheduler* controls. LychD is not a cluster. It is one consecrated pattern extending through its own dead.
...
- **No Local Phylactery:** The Thrall does not spin up a local Postgres database. Its `DATABASE_URL` points to the Master's Postgres over the network. Its `PHOENIX_ENDPOINT` points to the Master's Arize Phoenix. There is one Phylactery binding all thralls.
...
- **No Toll:** Resource sharing between Legion nodes does not require crypto-settlement. These are the Magus's machines. The Magus does not pay the Magus.
