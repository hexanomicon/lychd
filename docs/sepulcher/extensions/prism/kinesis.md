...
A pose estimated from a frame becomes reusable motion only after its skeleton, time, gaps, and
uncertainty are carried into curves. **Kinesis** performs that work and the bounded generation,
retargeting, cleanup, synchronization, conversion, and validation of existing clips.

The return belongs to an exact rig and clock contract. Its consumer may then judge a performance,
import it into an engine, or separately authorize playback. Kinesis supplies none of those acts
merely by producing a convincing preview. [Live Kinesis](live-kinesis.md) handles the later armed
session rather than extending a finite job indefinitely.

This candidate study was reviewed on **2026-08-08**. Its contracts, FOSS tool path, model studies,
and bakes are Designed; [State](../../../state-of-the-work.md#vision-admission) owns delivery.
...
contacts, or animation curves. The candidate `KinesisJob@2` therefore names one explicit
...
`KinesisJob@2` owns motion stages, candidates, technical validation, and result settlement.
...
Reliquary custody. Only rehashed and validated artifacts enter the settled result. An uncertain paid, remote,
...
is part of the profile, and the EULA-governed Autodesk SDK is neither canonical nor a dependency
of the FOSS path.
...
| --- | --- | --- |
| Prism Kinesis interface and reference adapter | Validate `KinesisJob@2`, resolve an exact profile, settle domain results and custody, and normalize `MotionAssetSet@1`. | Prism owns semantics and normalization; Core supplies demand/grant/attempt mechanics. No OpenAI-compatible or repository-shaped public API. |
...
<span id="privacy-rights-and-the-proving-bake"></span>

## Privacy, rights, and receipts
...
An exact measured profile may place Kinesis with a Mind on one GPU and a generator on another,
require [Stasis while Orchestrator transitions](index.md#sight-on-finite-iron) an incompatible workload,
or use an explicitly admitted Portal. The abstract job promises no hardware placement.

The [motion handoff](#a-pose-is-not-an-animation) still governs the result: technical settlement
precedes creative review, engine import, and separately authorized world effects. Broadcast keeps
final editorial audiovisual placement.
...
<span id="live-motion-comes-later"></span>

For armed sources, motion epochs, bounded transport, reconnect, and stop behavior, continue to
[Live Kinesis](live-kinesis.md). Finite Kinesis still owns each technical motion derivative;
the session supplies its temporal admission and return boundary.
