...
While the **[Soulforge](./soulforge.md)** builds the mind, The Riddle tests it. It moves beyond static benchmarks by subjecting models to live, execution-based trials curated to identify the best spirit for every specific capability.
...
2. **The intelligence floor:** If the Riddle proves that a 7B local Soulstone can solve a specific class of task with 95% accuracy, the system establishes an intelligence floor for it.
3. **Economic guardrail:** Following the laws of **[The Toll](./toll.md)**, the system is physically barred from expending cloud tokens on tasks that have been "solved" by local silicon. External Portals are reserved exclusively for frontier riddles that local hardware has historically failed.
...
!!! tip "The Logic-per-Watt Metric (Stillness)"
    An efficient system makes one clean, targeted cut instead of burning tokens on unbounded speculation. The Riddle tracks `Accuracy / VRAM_Occupancy` to find the models that do — identifying "Hidden Titans," small models that punch significantly above their weight class — ensuring the Lich remains lean, fast, and precise. This is the endgame of optimization: **Stillness**.
