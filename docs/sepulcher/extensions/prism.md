...
Sight is not a single model; it is an entire operational state. The Prism manifests the `vision.coven`, a collection of local **[Soulstones](../animator/soulstone.md)** rendered as **[Quadlet services](../../adr/08-containers.md)** and managed as a single, atomic unit by the **[Orchestrator](../../adr/23-orchestrator.md)**. A typical Prism Coven includes:

- **The Eye (`vlm.container`):** The primary Vision Language Model Soulstone. A powerful VLM may possess multiple capabilities, such as `capabilities=["vision-analysis", "ocr"]`.
...
- **The Animator (Provider):** When an Agent requires reasoning about an image, the Dispatcher resolves the `vision-analysis` capability to the active model-backed Animator. This is bound to the Agent as a Pydantic AI **`Model`**.
