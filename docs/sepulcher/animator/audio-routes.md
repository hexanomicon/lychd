...
This page maps replaceable Linux audio devices and host routes beneath several semantic owners. It
lives beside Animator because it concerns concrete service and device mechanisms; that placement
grants Animator no speech, music, body, presentation, or application judgment.
[Audio](../../adr/37-audio.md) retains capture consent, custody, transport, interruption, and
playback law.
...
STT/TTS adapter, playback receipt, live Riffmaw graph, or Familiar audio adapter. This is an
operational candidate map, not a host receipt or delivery claim.
...
latency-sensitive take, turn it on. Riffmaw keeps dry DI separate from the amp/cabinet print so a
later mix can repair the sound. The motherboard headphone output may remain ordinary desktop
...
6. Add Guitarix and Ardour or Reaper only after the interface is stable at 48 kHz; measure xruns
...
1. **Host adapter:** enumerate ALSA/PipeWire devices and retain route facts without granting
   capture; handles and PCM remain outside Graph state.
...
4. **Riffmaw capture take:** dry instrument DI or musical vocal under an exact device/clock/latency
