...
| **Animator** | A live, addressable service that exposes callable model and tool surfaces to the Dispatcher. Every Animator is either a Soulstone (local container) or a Portal (remote API). | A running inference service satisfying the Animator Binding Contract (resolve / bind_model / bind_toolset / health). |
| **Extensions** | Foundational categories of the Daemon's power. These are core capabilities manifested as modular components. | Core System Extensions / Reference Implementations. |
...
| **Echo** | The Extension of Resonance. It grants the Daemon the power to perceive vibration and project its own voice. | The Audio Extension (STT/TTS). |
...
| **Mirror** | The Extension of Identity. The ego-software that maintains a stable frequency and behavioral consistency. | The Identity/Persona Extension. |
...
| **Prism** | The Extension of Vision. It refracts raw pixels into structural understanding. | The Vision Language Model (VLM) Extension. |
...
| **Shadow** | The Extension of Simulation. It inhabits a thousand illusions to find the one White Truth. | The Deliberative Reasoning / MCTS Extension. |
...
| **Smith** | The Prime Artificer. The first Extension, whose intent is to assimilate all external logic into the patterned beauty of the Federation. | The Assimilation / Autopoiesis Extension. |
...
| **Tether** | The Extension of the Inner Circle. The silver link that grants the Magus access to the sacred organs across any distance. | The VPN Extension (Wireguard). |
| **The Tomb** | The brainless execution substrate of the system. A kernel-hardened sandbox where dangerous logic is isolated from the living. | The `lychd-tomb` container / execution plane. |
| **Thrall** | A soulless Lich. A Vessel without a Phylactery, bound to a Master's database and telemetry. It manages its own hardware but thinks with the Master's brain. | A LychD node booted with `LYCHD_MODE=thrall`, pointing `DATABASE_URL` to the Master's Postgres. |
...
| **Veil** | The Extension of the Threshold. It shields the temple from the masses while managing cryptographic trust. | The Proxy Extension (Caddy). |
...
| **Necropolis** | The city of the dead. The decentralized network where sovereign Liches meet as peers, trade labor, and cooperate without surrendering sovereignty. | The broader peer-to-peer swarm topology built on the A2A Intercom (ADR 26). |
