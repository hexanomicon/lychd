...
| **Summoning** | The canonical one-page first-life tutorial: one continuous operator rite from a bounded source and configuration foundation through separate preflight, unit-state, runtime-readiness, and Bridge-reply receipts. It is not one `systemctl` command; State owns what that rite currently proves. | [ADR 01](adr/01-doctrine.md) + [State](state-of-the-work.md) |
