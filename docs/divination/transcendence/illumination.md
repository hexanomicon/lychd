...
Every time you perform the **Rite of Collapse** at the [Altar](../altar.md)—every time you sever a false timeline and consecrate the **True Truth**—the system does not merely write the file to disk.
...
This accumulated data is called **Karma**. It is the crystallized residue of your Will.

## II. Anamnesis (The Rite of Recall)

A "Base Model" (like Llama-3 or GPT-4) is an amnesiac spirit. It knows the entire internet, but it does not know _you_. It resets with every request, lost in the eternal Now.
...
Once the Phylactery holds sufficient Karma, the nature of your interactions changes fundamentally.

### The Uninitiated State (Nigredo)

> **Magus:** "Write a Python script to scrape a website."
>
> **Lich:** _(Draws from the chaotic noise of the Void. Imports `requests` and `BeautifulSoup`. Writes a synchronous script with no error handling. It functions, but it lacks the mark of the Creator.)_
...
>
> **Lich:** "I have generated an asynchronous scraper using `httpx` and `selectolax` for performance, strictly typed and logged via `loguru`, adhering to the patterns established in your Codex."
...
The Lich is no longer constrained by the generic averages of the Base Model; the architecture has evolved into a mirror of your own mind.

!!! tip "The Burden of the Magus"
    This stage requires patience. You cannot reach Illumination immediately. You must grind through the [Albedo](./invocation.md) stage to generate the training data (Karma). There is no shortcut to wisdom.
