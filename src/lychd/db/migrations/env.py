...
def order_columns(
    ...
    cols_by_key: list[tuple[int, Column[Any] | Any]] = [
        (
    ...
    return ops.CreateTableOp(
        ...
        _namespace_metadata=op._namespace_metadata,  # type: ignore[protected-access]  # noqa: SLF001
        **op.kw,
    )
...
async def run_migrations_online() -> None:
    ...
    if connectable is None:  # type: ignore[unreachable]
        msg = "Could not get engine from config.  Review `alembic.ini`"
        raise RuntimeError(
            msg,
        )
