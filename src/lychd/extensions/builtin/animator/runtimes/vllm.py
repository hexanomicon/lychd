...
class VllmRuntimeAdapter:
    ...
    The adapter only owns the container envelope (image, port, IPC/network podman
    args) and the capability/probe plumbing. vLLM framework flags are never
    re-typed here; the operator's ``exec`` list is authoritative.
    """
    ...
    def plan(self, soulstone: SoulstoneConfig) -> RuntimePlan:
        """Plan the vLLM container envelope; framework flags come from ``exec`` verbatim."""
        stone = require_runtime_soulstone(
            soulstone,
            expected_type=VllmSoulstoneConfig,
            runtime=self.runtime,
