...
generator drains its replay buffer and completes — the read can never hang.
"""
...
def test_stream_projects_every_event_kind(altar_client: TestClient[Litestar], fake_services: SimpleNamespace) -> None:
    ...
    channel = sessions.channel(run_id)
    channel.emit("status", "weaving")
    channel.emit("token", "<b>ashes</b>")
    channel.emit("fragment", json.dumps({"key": "genui.plan_checklist", "params": {"title": "Rite", "steps": ["a"]}}))
    channel.emit("consent", consent_id)
    channel.emit("done", run_id)
...
def test_stream_closes_after_done(altar_client: TestClient[Litestar], fake_services: SimpleNamespace) -> None:
    ...
    sessions = fake_services.bridge_sessions
    channel = sessions.channel("run_done")
    channel.emit("status", "settling")
    channel.emit("done", "run_done")
