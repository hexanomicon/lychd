...
def test_snapshot_lists_created_session(altar_client: TestClient[Litestar]) -> None:
    created_response = altar_client.post("/api/v1/bridge/sessions")
...
def test_snapshot_reconstructs_selected_ledger_runs_without_requiring_a_live_channel(
    ...
    assert [run["run_id"] for run in active_runs] == [
        "run_selected",
        "run_without_process_channel",
    ]
    live, channel_less = active_runs
    assert (live["cursor"], live["content"], live["activity"]) == (2, "still speaking", "weaving")
    assert live["fragments"][0]["kind"] == "genui.plan_checklist"
    assert live["fragments"][0]["props"]["steps"] == ["listen"]
    assert (channel_less["cursor"], channel_less["content"], channel_less["activity"]) == (-1, "", "running")
    assert channel_less["fragments"] == []


def test_selected_session_never_projects_another_sessions_consent(
    altar_client: TestClient[Litestar],
    fake_services: SimpleNamespace,
) -> None:
    selected = _session(fake_services)
    ...
    async def _seed() -> None:
        for session_id, run_id, tool_name in (
            (selected.id, "run-selected-consent", "selected-tool"),
            (other.id, "run-other-consent", "other-tool"),
        ):
            await fake_services.ledger.create(
            ...
            await fake_services.consents.park(
                run_id=run_id,
                tool_name=tool_name,
                tool_call_id=f"call-{run_id}",
                call_ids=(f"call-{run_id}",),
                args={"secret": run_id},
    ...
    body = altar_client.get(f"/api/v1/bridge/sessions/{selected.id}").json()
...
def test_send_replay_returns_one_run_and_one_retained_turn(
    ...
    assert body["turn"]["content"] == payload["prompt"]
    ...
    assert fake_services.run_engine.submitted[0].prompt == payload["prompt"]
    assert fake_services.run_engine.submitted[0].sigil_name
    assert fake_services.run_engine.submitted[0].sigil_scopes
    assert fake_services.run_engine.exclusive_session_submissions == [True, True]
